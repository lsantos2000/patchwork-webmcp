...
Plan together and Action history share a separate session-only workspace, leaving Discover unchanged. `get_workspace` reads current choices; `propose_plan_revision` validates the revision, pins, and combined budget before displaying a proposal. Acceptance and undo are UI actions. Six tools exist across the app, with only the active workflow's tools registered. Native compatibility evidence for the two new tools is still pending; do not attribute them to the earlier video.

...
report={'file':output.name,'duration_seconds':actual,'under_three_minutes':actual is not None and actual<180,'resolution':'1920x1080','video_codec':'H.264','audio_codec':'AAC','voice':'Piper en_US-ljspeech-high, female synthetic narration; public-domain LJ Speech dataset','scene_count':len(scenes),'format':'Edited still-frame walkthrough with native WebMCP result excerpts and labeled automated UI captures, not a continuous screen recording','audio_measurements':re.findall(r'(?:mean|max)_volume: [^\n]+',probe.stderr),'public_url':'https://youtu.be/so9sDOxzeJY','review_required':'Confirm the public YouTube video remains audible, captioned, under three minutes, and playable while logged out.'}
